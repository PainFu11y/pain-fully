...
@Tag(name = "Мероприятия", description = "Контроллер для управления мероприятиями")
public class EventController {
    ...
    @Operation(summary = "Получить мероприятие по publicId")
    @GetMapping("/public/{publicId}")
    public ResponseEntity<EventDto> getPublicEvent(@PathVariable UUID publicId) {
        log.info("Received request to get public event by ID: {}", publicId);
        EventDto dto = eventService.getPublicEvent(publicId);
        log.info("Successfully fetched public event by ID: {}", publicId);
